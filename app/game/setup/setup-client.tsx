...
export default function GameSetupPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[linear-gradient(180deg,#edf4e6_0%,#f5f1e7_52%,#eef2e8_100%)] text-[#192117]">
      ...
      <div className="relative z-10 mx-auto min-h-screen w-full max-w-[1140px] px-4 pb-14 pt-5 sm:px-6 lg:px-8 lg:pt-6">
        <header className="flex items-center justify-between gap-4">
          <button
            type="button"
            onClick={handleBack}
            className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/70 px-3.5 py-1.5 text-sm font-semibold text-[#2b3724] shadow-[0_10px_24px_rgba(27,44,20,0.08)] transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-white"
          >
            <ArrowLeft className="h-4 w-4" />
            Back
          </button>

          <div className="hidden rounded-full border border-white/70 bg-white/60 px-3.5 py-1.5 text-[0.65rem] font-semibold uppercase tracking-[0.28em] text-[#5b6851] shadow-[0_10px_24px_rgba(27,44,20,0.05)] sm:block">
            Game setup
          </div>
          ...
          <Link
            href="/pro"
            prefetch
            className="inline-flex items-center gap-2 rounded-full border border-[#cfe0ba] bg-[#eef7e2] px-3.5 py-1.5 text-sm font-semibold text-[#4f7d3e] shadow-[0_10px_24px_rgba(27,44,20,0.05)] transition hover:-translate-y-0.5 hover:border-[#b9d6a1] hover:bg-[#e6f3d6]"
          >
            Pro rooms
          </Link>
        </header>
        ...
        <section className="mx-auto mt-10 max-w-3xl text-center">
          <h1 className="text-3xl font-semibold tracking-tight text-[#182015] sm:text-4xl lg:text-5xl">Choose your atmosphere</h1>
          <p className="mt-4 text-sm leading-7 text-[#5d6755] sm:text-base">Pick a room and start your match.</p>
        </section>

        <section className="relative mx-auto mt-10 w-full max-w-[1060px]">
          <div className="relative h-[360px] overflow-hidden rounded-[32px] border border-white/50 shadow-[0_30px_76px_rgba(39,59,30,0.22)] sm:h-[400px] lg:h-[440px]">
            ...
            <div className="absolute left-4 top-4 z-20 rounded-full border border-white/15 bg-black/18 px-3.5 py-1.5 text-xs font-medium text-white backdrop-blur-md sm:left-6 sm:top-6 sm:text-sm">
              {selectedRoomConfig.label}
            </div>
            <div className="absolute right-4 top-4 z-20 rounded-full border border-emerald-200/30 bg-emerald-950/35 px-3.5 py-1.5 text-xs font-medium text-emerald-50 backdrop-blur-md sm:right-6 sm:top-6 sm:text-sm">
              AI Coach Ready
            </div>
            ...
            <div className="absolute left-1/2 top-[51%] z-20 w-[84%] max-w-[660px] -translate-x-1/2 -translate-y-1/2 rotate-[2deg] sm:w-[80%] lg:left-[58%] lg:w-[64%]">
              <BoardPreview />
            </div>

            <div className="absolute bottom-5 left-5 z-30 max-w-[24rem] text-white sm:bottom-7 sm:left-7">
              <p className="text-2xl font-semibold tracking-tight sm:text-3xl">{selectedRoomConfig.label}</p>
              <p className="mt-2 max-w-md text-sm leading-6 text-white/78 sm:text-base">{selectedRoomConfig.description}</p>
            </div>
          </div>
        </section>

        <section className="mx-auto mt-12 w-full max-w-[1060px]">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-[0.68rem] uppercase tracking-[0.28em] text-[#6c7b61]">Vibe rooms</p>
              <h2 className="mt-2 text-xl font-semibold tracking-tight text-[#182015]">Choose the room mood.</h2>
            </div>
            {subscription.isActive ? (
              <Badge className="border-[#a7c994] bg-[#edf6e5] px-3.5 py-1.5 text-xs font-semibold text-[#5d8f49] sm:text-sm">
                Pro active · {formatTrialRemaining(subscription.remainingMs)}
              </Badge>
            ) : (
              <Badge className="border-[#d8ccb8] bg-[#f6f1e5] px-3.5 py-1.5 text-xs font-semibold text-[#6e6a5d] sm:text-sm">
                ...
                >
                  <div className="relative h-36">
                    ...
                    <div className="absolute left-4 top-4">
                      <Badge
                        className={cn(
                          "px-2.5 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.22em]",
                          active
                    ...
                    <div className="absolute right-4 top-4">
                      {locked ? (
                        <Badge className="border-[#e0c58f]/50 bg-[#fff0d4]/92 px-2.5 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.22em] text-[#8d6a2b]">
                          Locked
                        </Badge>
                      ) : active ? (
                        <Badge className="border-[#a7c994]/35 bg-[#edf6e5]/92 px-2.5 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.22em] text-[#5d8f49]">
                          Selected
                        </Badge>
                      ) : null}
                    </div>
                    ...
                    {locked ? (
                      <div className="absolute inset-0 flex items-center justify-center bg-[#0c120d]/22">
                        <div className="flex items-center gap-2 rounded-full border border-white/18 bg-black/20 px-3.5 py-1.5 text-sm font-medium text-white backdrop-blur-md">
                    ...
                    <div className="absolute bottom-4 left-4 right-4 text-white">
                      <p className="text-lg font-semibold tracking-tight">{room.label}</p>
        ...
        <section className="mt-12">
          <div className="mx-auto grid w-full max-w-[860px] gap-4 md:grid-cols-3">
            {MODE_OPTIONS.map((item) => {
              ...
              return (
                <motion.button
                  ...
                  aria-pressed={active}
                  className={cn(
                    "flex h-16 w-full items-center justify-center rounded-full border px-5 text-center text-xs font-semibold tracking-[0.24em] transition duration-300 sm:h-20 sm:text-sm",
                    active
        ...
        {mode === "bot" ? (
          <motion.section
            ...
            className="mt-12"
          >
            <div className="mx-auto grid w-full max-w-[800px] gap-4 sm:grid-cols-3">
              {PERSONALITY_OPTIONS.map((item) => {
                ...
                return (
                  <motion.button
                    ...
                    aria-pressed={active}
                    className={cn(
                      "flex min-h-[132px] w-full flex-col items-start rounded-[28px] border p-4 text-left transition duration-300",
                      active
                  ...
                  >
                    ...
                    <span className="mt-4 text-[0.72rem] font-semibold tracking-[0.26em] text-[#263325]">{item.title}</span>
        ...
        {notice ? (
          <div className="mx-auto mt-8 max-w-[900px] rounded-[22px] border border-[#d9d0bf] bg-[#fffaf3] px-4 py-3 text-sm leading-6 text-[#5f665b]">
            {notice}
          </div>
        ) : null}

        <section className="mt-auto flex justify-center pt-14 pb-4">
          <motion.button
            type="button"
            ...
            whileTap={{ scale: 0.98 }}
            className={cn(
              "h-14 w-[300px] max-w-full rounded-full bg-[linear-gradient(180deg,#79b95c_0%,#5f9e47_100%)] px-7 text-base font-semibold text-white shadow-[0_24px_54px_rgba(89,151,64,0.32)] transition duration-300",
...
function BoardPreview() {
  return (
    <div className="relative mx-auto aspect-[1.72/1] w-full max-w-[660px]">
      <div className="absolute inset-[8%] rounded-[30px] bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.2),transparent_28%),linear-gradient(135deg,#9a6d3a_0%,#6f4527_100%)] shadow-[0_36px_80px_rgba(0,0,0,0.44)]" />
      <div className="absolute inset-[10.25%] rounded-[24px] border border-white/15 bg-[linear-gradient(135deg,#efd5a4_0%,#d9b06e_22%,#bd874d_52%,#8d5c33_100%)] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.18)]">
        ...
        <div className="absolute inset-[6%] grid grid-cols-[1fr_0.12fr_1fr] items-stretch gap-2">
          ...
          <div className="relative rounded-[16px] bg-[linear-gradient(180deg,#7f5831_0%,#5d3c22_100%)] shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]">
      ...
      <div className="absolute inset-x-[20%] bottom-[2%] h-8 rounded-full bg-black/35 blur-3xl" />
    </div>
  );
}
    ...
    <div className={cn("absolute flex flex-col-reverse items-center", className)}>
      {Array.from({ length: count }).map((_, index) => (
        <span
          ...
          className={cn("h-6 w-6 rounded-full border shadow-[0_4px_10px_rgba(0,0,0,0.28)] sm:h-7 sm:w-7", fill, color === "light" ? "border-white/65" : "border-white/15")}
          style={{
